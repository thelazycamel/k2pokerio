...
beforeEach(() => {
  ...
  let initialState = {
    page: {tabs: {}, links: {}},
    game: {
      best_cards: ["Kc", "Kh", "2c", "3s", "4h"],
      cards: ["Kc","Kh"],
      hand_description: "Pair",
      other_player_status: "new",
      player_status: "new",
      status: "flop",
      table_cards: ["2c","3s","4h"],
      result: {
        lose_description: "",
        other_player_cards: [],
        player_cards: [],
...
describe("Game States: The Flop", () => {
  ...
  test('The best hand show show a status', () => {
    expect(gameComponent.find("#best-hand").text()).toMatch("Pair");
  });

  test('The #table-cards should contain 3 cards', () => {
    expect(gameComponent.find("#table-cards .card").length).toEqual(3);
